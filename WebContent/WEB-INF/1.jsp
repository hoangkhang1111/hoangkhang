...
<body style="background-image: url(image/1.jpg);" >
	...
	<form >
		<br></br>
			<% 
				String kotoba1 = request.getParameter("kotoba");
			...
			<td>
				...
				<%
					if(request.getParameter("kotoba") == null  && count != 0){ %>
						<label>endgame</label>
							//Alert("khang");
					<%
						count += 1;
				...
				<% 	}else{
						int ko1 = kotoba1.length() - 1;
						int ko2 = kotoba2.length() - 1;
						if(request.getParameter("kotoba2") != null){

							if(kotoba1.substring(ko1).equals("n") == true || new ConnectDB().Kiemtratontaiendgame(kotoba1) != null){ %>
								<label>endgame</label>
							<% //Alert("endgame");								
							}if((kotoba1.substring(0)).equals(kotoba2.substring(ko2)) != true){ %>
								<label>loigame</label>
							<%
								//Alert("loigame");
							}else{
								if((new ConnectDB().Kiemtratontai(kotoba2)) == null) {
									int sutetasu = 1;
									Shisutemu shi = new Shisutemu( kotoba1, sutetasu);
